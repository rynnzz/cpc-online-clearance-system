  <template>
    <div class="flex">
      ...
      <div :class="['bg-gray-800 text-white transition-all duration-300', { 'w-82': isOpen, 'w-20': !isOpen }]" 
        ...
        <ul v-if="userRole.includes('teacher')">
          ...
          <li
            v-if="userRole.some(role => ['Full Time', 'Part Time'].includes(role))"
    ...
    v-if="
      ...
      userRole.some(role => 
        ['Department Head - BSIT', 'Department Head - BEED', 'Department Head - BSHM', 'Department Head - BSED', 'Accounting', 'Librarian', 'SAO/SSG Adviser', 'Guidance Counselor', 'Registrar', 'Clinic'].includes(role)
      )
    "
  ...
  <hr class="my-4 border-gray-700" />
  <li 
            :class="['flex items-center px-6 py-3 hover:bg-gray-700 cursor-pointer text-lg', { 'bg-gray-600': activeItem === 'teacher-profile' }]"
            @click="navigateTo('teacher-profile')"
          >
          <i class="fa-solid fa-gear mr-3"></i>
            <span v-if="isOpen">Account Settings</span>
          </li>
